...
ITEMS = [
    ...
    ("trb3um3y3093hnmk0qn8gxsyl0migogp", "Walt Disco and Ellie Mason album session 2025"),
    ...
    ("w0yju4t3ltqrrvxypntncvb78sh85uff", "Mouth Culture with Jamie Ward 2025"),
    ...
    ("t1g9v23hy6hr5pohtehq5qm3yfviyatf", "Dizzee Rascal in the control room 2024"),
    ...
    ("vblel1zxlcz3dypaohp3ckeejqq97w7t", ""),
    ...
    ("dg1l8i0du0kfrjruujmoagpw63x5277s", "Alex Gordon micing our 2048 Reverb-Trem"),
    ...
    ("imd5gynr5bj47t3e9d8o16fcdw9ems1d", "Songwriting session 2024"),
    ...
    ("5e6dulurltxeuy2vb79wxqbz9jddgw4o", "Danny Addison Album recording 2024"),
    ...
    ("uu3iedqhksl9jtyq6mv2axok63nlu58a", "Saint Agnes live video session with BlueJuice 2025"),
    ("pyyv1s6v9nabrwbxb8t9v6wf25hq59tc", "Saint Agnes Live Video session 2025"),
    ("0rgx3rbjqtsdzwmjzl4qad13sp0bvfbh", "Blue Juice Visuals capturing a Saint Agnes session 2025"),
...
ALT_OVERRIDES = {
    "y2zlsjtm9qlkk0vnmxxrrmdfzdkuzb6h": "Producer working at the Salvation Studios mixing console",
    "ir9rrv9rno9ogft9zmoj0xx1ukmlsng7": "Two people in the Salvation Studios control room",
    "vblel1zxlcz3dypaohp3ckeejqq97w7t": "Vocal ensemble recording in the Salvation Studios live room",
    "vri0dw8x13gxrudmegc7ag0uzf8pn86b": "Band gathered after a Salvation Studios session",
    "jdrlqiob9pni0idohdd26m4a2d2mkbix": "Keyboard player performing in the Salvation Studios live room",
    "w41kjxw3x5gnjthq9wxoq5zj8cvkupb8": "Engineer working at the Salvation Studios mixing console",
    "44ej5xmcsyh3jwjwni7p2vhmposijhcf": "Guitarist seated in the Salvation Studios control room",
    "hakvli8o6z2j5j6fysngvfyg1k96rhng": "Three musicians performing together at Salvation Studios",
    "a8eskaw92a3n406e1gmn17d5jihqy5e3": "Guitarist playing a semi-hollow electric guitar at Salvation Studios",
    "s4ubs3lrunfpzh9z78ked3pagdwj7yqe": "Producer working with a synthesizer in the control room",
    "hiwegmysrfty80a0a7xn5f9cb6t1fgqh": "Camera operator filming a Salvation Studios session",
    "rnocynjnkyadd8avszlt9uks6ulpuc3a": "Vocalist performing with a string player at Salvation Studios",
    "corusuli96io8er5tudxiviulb3nt8w0": "Black-and-white portrait of a musician wearing headphones",
}

LOCAL_FILES = {
    "trb3um3y3093hnmk0qn8gxsyl0migogp": OUT_DIR / "home-walt-disco-and-ellie-mason-album-session-2025.webp",
}


def existing_items() -> dict[str, dict[str, object]]:
    if not MANIFEST.exists():
        return {}
    return {item["box_id"]: item for item in json.loads(MANIFEST.read_text(encoding="utf-8"))}


EXISTING_BY_ID = existing_items()
...
def import_item(item: tuple[int, str, str]) -> dict[str, object]:
    ...
    display_caption = re.sub(r"(?:\s+in)?\s+(?:19|20)\d{2}$", "", caption)
    slug = slugify(caption, f"gallery-{idx:02d}-{box_id[:8]}")
    existing = EXISTING_BY_ID.get(box_id)
    existing_path = ROOT / str(existing["src"]).lstrip("/") if existing else None
    out_path = LOCAL_FILES.get(box_id) or (existing_path if existing_path and existing_path.exists() else None)
    out_path = out_path or OUT_DIR / f"{idx:02d}-{slug}.webp"
